...
export default function Sidebar({ currentView, setCurrentView }: SidebarProps) {
  const menuItems: { id: View; label: string; icon: React.ReactNode }[] = [
    { id: 'shipments', label: 'Active Shipments', icon: <Truck size={20} /> },
    { id: 'assignments', label: 'Driver Assignments', icon: <Users size={20} /> },
  ];
  ...
  return (
    <aside className="w-64 bg-slate-900 text-white h-full fixed top-0 left-0 overflow-y-auto shadow-xl z-10 flex flex-col">
      <div className="p-6 border-b border-slate-800">
        ...
        <p className="text-xs text-slate-400 mt-1">Dispatcher Portal</p>
      </div>
      ...
      <div className="p-4 border-t border-slate-800">
        <div className="flex items-center">
            <div className="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center text-xs font-bold">DP</div>
            <div className="ml-3">
